...
export const Card = ({
  ...
  const isWoDCard = title === 'Workshop on Demand';
  ...
  return (
    <ResponsiveContext.Consumer>
      {(size) => (
        <GrommetCard
          background={isWoDCard && hover ? '#263040' : null}
          elevation="medium"
          margin="medium"
          flex="grow"
          style={{ cursor: 'pointer' }}
          basis={size === 'small' ? 'auto' : bases[width]}
          onMouseOver={() => isWoDCard && setHover(true)}
          onMouseLeave={() => isWoDCard && setHover(false)}
          {...rest}
          /* eslint-disable */
          onMouseUp={() => {
            link && link.match(/^\//g)
